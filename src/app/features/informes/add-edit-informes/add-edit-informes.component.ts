...
export class AddEditInformesComponent implements OnInit {
  ...
  cancelar() {
   ...
   //metodo que genera un informe de desempeño y lo actualoza con los contenidos
  generarInforme() {
    const alumnoInf: AlumnoInforme = {
      id: this.idAlumno,
    };
    const cursoinf: CursoInforme = {
    ...
    console.log(informeNuevo);

    this._informesService.save(informeNuevo).subscribe((data) => {
      console.log(data);
      ...
      this.contenidos.forEach( element => {
        ...
        this._informesService.asignarContenido(this.idInforme, element.id ).subscribe((data) => {
          console.log(data);
        },
        error => {
